...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if nc and nc != self.yaml.get("nc", 80):
            LOGGER.warning(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # update yaml
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg="yolo11n-seg.yaml", ch=3, nc=None, verbose=True):
        ...
        Args:
            ...
            verbose (bool): Whether to display model information.
        """
        # ---------------------- 改用标准库解析YAML（彻底解决导入错误） ----------------------
        if nc is None:
            # 1. 导入标准yaml库 + 正确路径的check_yaml
            import yaml
            try:
                from ultralytics.utils.checks import check_yaml
            except ImportError:
                # 极端情况：手动实现check_yaml（确保文件存在）
                import os
                def check_yaml(path):
            ...
            if isinstance(cfg, dict):
                ...
                cfg_path = check_yaml(cfg) if cfg else ""
                with open(cfg_path, encoding="utf-8") as f:
                    cfg_dict = yaml.safe_load(f) if cfg_path else {}
            # 3. 读取nc，兜底80
            nc = cfg_dict.get("nc", 80)
        # -------------------------------------------------------------------------------------
        ...
        # 确保self.yaml有nc键
        if hasattr(self, "yaml") and "nc" not in self.yaml:
            self.yaml["nc"] = nc
        # 一致性检查（用标准warning）
        if hasattr(self, "yaml") and nc != self.yaml.get("nc", 80):
            import warnings
            warnings.warn(f"Updating model nc from {self.yaml.get('nc', 80)} to {nc}")
            self.yaml["nc"] = nc
...
from ultralytics.nn.modules.ghost_seg import GhostModuleSeg
...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            C2fCIB,
            A2C2f,
            GhostModuleSeg,  # 保留在base_modules中，无特殊处理
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            c1, c2 = ch[f], args[0]
            # ---------------------- 删除：跳过GhostModuleSeg缩放的特殊逻辑 ----------------------
            if c2 != nc:  # 恢复默认通道缩放逻辑（对所有base_modules生效）
